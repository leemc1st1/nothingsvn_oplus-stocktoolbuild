...
if [[ $regionTYPE == "Domestic" ]]; then 
    ...
    aria2c_with_github_token -q -d "$work_dir/bin/package/Universal/GMS/product_privapp/GoogleVelvet_CTS/" -o GoogleVelvet_CTS.apk https://github.com/tiencv2006/nothingsvn_oplus-toolbuild/releases/download/oplus/GoogleVelvet_CTS.apk && echo "[INFO] - Get File Successfully"
    aria2c_with_github_token -q -d "$MAIN_FOLDER/my_product/app/" -o Photos.apk https://github.com/tiencv2006/nothingsvn_oplus-toolbuild/releases/download/oplus/Photos.apk && echo "[INFO] - Get File Successfully"
    rm -rf $disper1
    rm -rf $disper2
