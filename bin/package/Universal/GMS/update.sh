...
if [[ $regionTYPE == "Domestic" ]]; then 
    ...
    aria2c -q -d "$work_dir/bin/package/Universal/GMS/product_privapp/GoogleVelvet_CTS/" -o GoogleVelvet_CTS.apk https://github.com/tiencv2006/NothingsVN-BuildExt/releases/download/oplus/GoogleVelvet_CTS.apk && echo "[INFO] - Get File Successfully"
    aria2c -q -d "$MAIN_FOLDER/my_product/app/" -o Photos.apk https://github.com/tiencv2006/NothingsVN-BuildExt/releases/download/oplus/Photos.apk && echo "[INFO] - Get File Successfully"
    rm -rf $disper1
    rm -rf $disper2
